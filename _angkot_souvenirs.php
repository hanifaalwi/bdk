<?php
    ...
    $result=  "SELECT detail_souvenir.id_souvenir,angkot.id,angkot.route_color, angkot.destination, detail_souvenir.id_angkot,souvenir.name,souvenir.id,detail_souvenir.lat,detail_souvenir.lng,detail_souvenir.description, ST_X(ST_Centroid(souvenir.geom)) AS longitude, ST_Y(ST_CENTROID(souvenir.geom)) As latitude FROM detail_souvenir left join angkot on detail_souvenir.id_angkot=angkot.id left join souvenir on detail_souvenir.id_souvenir=souvenir.id where souvenir.id='$id'";

        $tes=mysqli_query($conn, $result);
